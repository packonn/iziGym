	...
	return (
		<div className="container  grid grid-cols-4 gap-x-6">
			<div className="lg:col-span-3 col-span-4">
				<Title title={title} subtitle={subtitle} color="secondary" />

				<div
					dangerouslySetInnerHTML={{ __html: content }}
					className="font-roboto dropdownContentCustomHtml text-16"
				></div>
				{collapse && (
					...
					{showVideo && videoURL && (
						<div>
							...
							<div className="player-wrapper overflow-hidden p-[0px]  bg-black ">
								<VideoPlayer url={videoURL} isPlaying={false} />
							</div>
						</div>
					)}
					{gallery && (
						<div className="mt-10">
							...
							<SwiperGallery
								data={[
									...gallery.map((item) => item.sourceUrl),
								]}
